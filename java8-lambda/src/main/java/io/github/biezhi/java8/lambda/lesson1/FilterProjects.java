...
public class FilterProjects {
    ...
    public static void main(String[] args) {
        ...
        data.add(Project.builder().name("Blade").language("java").author("biezhi").stars(3500)
                .description("Lightning fast and elegant mvc framework for Java8").build());

        data.add(Project.builder().name("Tale").language("java").author("biezhi").stars(2600)
                .description("Best beautiful java blog, worth a try").build());

        data.add(Project.builder().name("Vue.js").language("js").author("yyx990803").stars(83000)
                .description("A progressive, incrementally-adoptable JavaScript framework for building UI on the web.")
                .build());

        data.add(Project.builder().name("Flask").language("python").author("pallets").stars(10500)
                .description("The Python micro framework for building web applications").build());

        data.add(Project.builder().name("Elves").language("java").author("biezhi").stars(200).description("Spider")
                .build());
        ...
        projects = filterProjects(data, new ProjectLanguagePredicate("java"));
        ...
        Predicate<Project> predicate = (Project project) -> project.getStars() > 1000;
        List<Project> filter2 = filter(data, predicate);

        List<Project> filter = filter(data, project -> project.getStars() > 1000);

        Function<? super Project, ? extends Integer> keyExtractor = Project::getStars;
        Comparator<Project> comparing = Comparator.comparing(keyExtractor);
        data.sort(comparing);
