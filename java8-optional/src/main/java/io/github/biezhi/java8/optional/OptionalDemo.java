...
public class OptionalDemo {
    ...
    public static void main(String[] args) {
        ...
        OptionalDemo oDemo = new OptionalDemo();
        oDemo.createOptional();
        oDemo.map();
        ...
        // try {
        // address1 = user.getOptAddress().filter(address ->
        // address.getDoor().contains("878"))
    ...
    public static String getStreet(Optional<User> user, int minAge) {
        return user.filter(u -> u.getAge() >= minAge).flatMap(User::getOptAddress).map(Address::getStreet).orElse("没有");
    }
    ...
    public int readPoint(Properties props, String name) {
        return Optional.ofNullable(props.getProperty(name)).flatMap(OptionalDemo::parseInt).filter(i -> i > 0)
                .orElse(0);
    }
